...
$defaults = @()
$defaultsScript = Join-Path $repoRoot "experiment\splatfacto_mcmc.defaults.ps1"
if (Test-Path -LiteralPath $defaultsScript) {
  . $defaultsScript
  if (Get-Command Get-SplatfactoMcmcDefaults -ErrorAction SilentlyContinue) {
    $defaults = @(Get-SplatfactoMcmcDefaults)
  }
}

if ($defaults.Count -eq 0) {
  $fallbackDefaults = Join-Path $repoRoot "experiment\splatfacto_big.defaults.ps1"
  if (Test-Path -LiteralPath $fallbackDefaults) {
    . $fallbackDefaults
