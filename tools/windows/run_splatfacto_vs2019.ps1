...
$extraArgsQuoted = $ExtraArgs | ForEach-Object { '"' + $_.Replace('"', '""') + '"' }
$extraArgsJoined = $extraArgsQuoted -join ' '

$dataPathFull = (Resolve-Path -LiteralPath $DataPath).Path
$isBlender = Test-Path (Join-Path $dataPathFull "transforms_train.json")
$isColmap = (
  (Test-Path (Join-Path $dataPathFull "colmap\sparse\0\cameras.txt")) -or
  (Test-Path (Join-Path $dataPathFull "colmap\sparse\0\cameras.bin"))
)
$dataset = if ($isBlender) { "blender-data" } elseif ($isColmap) { "colmap" } else { "nerfstudio-data" }
...
$cmdLines = @(
  ...
  """" + $escaped.NsTrainExe + """" + " splatfacto %DEFAULT_ARGS% " + $extraArgsJoined + " " + $dataset + " --data """ + $escaped.DataPath + """"
)

$tempCmd = Join-Path $env:TEMP ("ns-run-splatfacto-" + [guid]::NewGuid().ToString("N") + ".cmd")
try {
